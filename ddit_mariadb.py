...
try:
  ...
  for file_path in glob.glob(os.path.join('data/', '*.txt')):
    with open(file_path,'r') as f:
      ...
      # compile table sql
      create_sql = 'create table if not exists ' + table_name + ' (' + create_column + ');'
      ...
      f.seek(0, 0)
      next(f)
      #load_data = "LOAD DATA LOCAL INFILE '" + file_abspath + "' INTO TABLE " + table_name + " FIELDS TERMINATED BY '\t' LINES TERMINATED BY '\n' IGNORE 1 LINES (" + load_data_col + ")"
      load_data = "LOAD DATA LOCAL INFILE '" + file_abspath + "' INTO TABLE " + table_name + " FIELDS TERMINATED BY '\t' LINES TERMINATED BY '\n' IGNORE 1 LINES"
      cur.execute(load_data)
      conn.commit()
      
except mariadb.Error as ex: 
  print(f"An error occurred while connecting to database: {ex}")
  sys.exit(1) 
